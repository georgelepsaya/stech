        ...
        <div class="flex justify-between items-center">
            ...
            @can('create', 'App\Models\Article')
                <div class="relative inline-block text-left group">
                    <a href="{{ route('feed.create_article') }}" class="text-gray-400 font-medium hover:text-gray-200 cursor-pointer transition-colors duration-200 focus:outline-none">{{__('feed.create_article_title')}}</a>
                </div>
            @endcan
        </div>
    </x-slot>
                    ...
                    <div class="p-4 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg dark:text-gray-200 text-gray-800">
                        <h4 class="flex flex-row justify-between text-lg font-medium">
                            ...
                            <span class="text-gray-400 flex items-center">
                                ...
                                {{ __('general.by') }} <a href="{{ route('users.show', ['id' => $article->author()->get()[0]->id]) }}" class="ml-1 text-gray-400 font-bold hover:text-blue-500">{{ $article->author()->get()[0]->name . (($article->author()->get()[0]->blocked)? ' [blocked]' : '') }}</a></span>
                        </h4>
