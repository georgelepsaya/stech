    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center button-group">
                @can('update', $review)
                    <a class="rounded-md bg-gray-500 text-gray-900 px-3 text-md edit-button" href="{{ route('reviews.edit', $review->id) }}">{{__('reviews.edit')}}</a>
                @endcan
                @can('delete', $review)
                    <form action="{{ route('reviews.delete', ['id' => $review->id]) }}" method="post" enctype="application/x-www-form-urlencoded">
                        @csrf
                        @method('delete')
                        <input type="submit" name="delete" class="cursor-pointer rounded-md bg-gray-500 text-gray-900 px-3 text-md delete-button" value="{{__('reviews.delete')}}">
                    </form>
                @endcan
            </div>
        </div>
