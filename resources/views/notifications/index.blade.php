<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h1 class="text-lg font-bold">{{ __('notifications.unread_ones') }} - <span id="notifications-count">{{count(array_filter($notifications->toArray(), function($item) {return !$item['read'];}))}}</span></h1>
                                ...
                                <div id="notification-{{$notification->id}}" class="notification-item w-full p-4 bg-white {{$notification->read ? 'dark:bg-gray-800' : 'dark:bg-gray-700'}} overflow-hidden shadow-sm sm:rounded-lg text-gray-200">
                                    <h4 class="flex flex-row justify-between items-center text-lg font-medium">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span id="new-badge-{{$notification->id}}" class="{{$notification->read ? 'hidden' : ''}} bg-blue-500 rounded-lg ml-4 px-2 text-sm">{{ __('notifications.new') }}</span>
                                        </div>
                                    </h4>
                                </div>
